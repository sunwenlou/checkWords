...
$(function() {
	...
	formData = $('#dgr')
			.datagrid(
					{
						url : ctx + "/smsCount/qrySms?sendType=marke",
						fit : true,
						fitColumns : true,
						border : false,
						pagination : true,
...
function queryForm(){
	...
	$.ajax({
	        type: "POST",
	        url:ctx+"/smsCount/countForAdmin?sendType=marke",
	        async: false,
...
//							value = value.replace(/&gt;/g,"》"); 
							value = value.replace(/'/g,"\"");
							var alertMsg = value.replace(/"/g,"");
							var content = "<a href='javascript:showMsg(\""+alertMsg+"\");' title='"+value+"' titile='"+ value + "' class='content'>"+ showtitle + "</a>";
							return content;
						}
					}
		       	},
