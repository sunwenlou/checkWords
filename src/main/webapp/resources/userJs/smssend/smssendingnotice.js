...
//			}
	...
	$('#accountid').combobox({
		...
		url : ctx + '/smsSendingRec/getAccount?smsAccountType=notice',
		editable:false,
	...
	$('#recoretable').datagrid({
		...
		remoteSort : true,
		striped : true,
		url : ctx + '/smsSendingRec/lisNotice',
		columns : [ [ {
			field : 'smsSystemName',
			title : '系统',
			width : getWidth(3)
...

		{
			...
			formatter : function(value, row, index) {
				...
				if ('N' == row.returnSuccess||'F' == row.sendSts) {
...
/*	$
	.ajax({
		type : "POST",
		url : ctx + "/smsSendingRec/countNoticeOrCheck?tag=1",
		async : false,
